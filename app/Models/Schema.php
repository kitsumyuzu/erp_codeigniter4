...
class Schema extends Model {
	...
	/* ---------------------------------------------------------------------- */
		...
		public function visual_table_kas($userId)
		{
			return $this -> db -> table('murid') -> select('murid_nama_depan, murid_nama_belakang') -> where('id_murid', $userId) -> get() -> getRowArray();
		}
		...
		public function visual_table_orderby($table, $columName, $columOrder) {

			if ($columOrder !== 'DESC')
			{
				return $this -> db -> table($table) -> orderBy($columName, 'ASC') -> get() -> getResultArray();
			}
			
			else if ($columOrder !== 'ASC')
			{
				return $this -> db -> table($table) -> orderBy($columName, 'DESC') -> get() -> getResultArray();
			}
